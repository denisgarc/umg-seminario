...
public class UnidadController extends HttpServlet {
    ...
    @EJB
    private UnidadFacade unidadService;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            if(request.getMethod() == "POST"){
                Short codigo = Short.parseShort(request.getParameter("idUnidad"));
                String descripcion = request.getParameter("descripcion");
                String abreviatura = request.getParameter("abreviatura");
                String activo = request.getParameter("activo");

                Unidad modelo = new Unidad();
                modelo.setIdUnidad(codigo);
                modelo.setDescripcion(descripcion);
                modelo.setAbreviatura(abreviatura);
                modelo.setActivo(activo);

                if(codigo == 0){
                    unidadService.create(modelo);
                } else {
