...
public class EspecializacionController extends HttpServlet {
    ...
    @EJB
    private EspecializacionFacade especializacionService;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            if(request.getMethod() == "POST"){
                Short codigo = Short.parseShort(request.getParameter("idEspecializacion"));
                String descripcion = request.getParameter("descripcion");
                String activo = request.getParameter("activo");

                Especializacion modelo = new Especializacion();
                modelo.setIdEspecializacion(codigo);
                modelo.setDescripcion(descripcion);
                modelo.setActivo(activo);

                if(codigo == 0){
                    especializacionService.create(modelo);
                } else {
